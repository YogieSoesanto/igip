<?php
class PrintModelSuratJalan extends PrintModel{
	...
	/*-----------------------------attribute-------------------------------*/
		...
		const CONST_MAKS_LEN_NAMA_BARANG = 73;
	/*--------------------------setter dan getter--------------------------*/
	/*---------------------------public logic------------------------------*/
		public function suratJalan($idF, $tipeExcel = self::CONST_TIPE_EXCEL_STORE){
			$fj = FakturJual::find($idF);
			if($fj == NULL){
	...
	/*----------------------------private logic----------------------------*/
		private function createData($obj){
			...
			$row = $this->makeHeaderFaktur($row, "Surat Jalan", $obj['header'][0], $obj['header'][1]);
		...
		private function makeHeaderFaktur($startRow, $namaFaktur, $namaPelanggan, $tanggalFaktur){
			...
			$this->tulis("Nama Pelanggan : " . $namaPelanggan, $colTemp.$row);
		...
		private function judulDariBodyFaktur($colAwal, $colAkhir, $sr){
			...
			$this->fontSize("12", $colAwal[0].$sr .":".$colAkhir[2].$sr);

			$this->fontWeight("bold", $colAwal[0].$sr .":".$colAkhir[2].$sr);

			$this->hAlign("center", $colAwal[0].$sr .":".$colAkhir[2].$sr);
			$this->vAlign("center", $colAwal[0].$sr .":".$colAkhir[2].$sr);
		...
		private function buatBodyFakturPerBaris($dataPerBaris, $colAwal, $colAkhir, $sr, $angkaLooping, $isTutup = false){
			...
			$this->fontSize("11",$colAwal[0].$sr .":".$colAkhir[2].$sr2);

			$this->hAlign("center", $colAwal[0].$sr .":".$colAkhir[2].$sr2);
			$this->hAlign("left", $colAwal[1].$sr .":".$colAkhir[1].$sr2);
			

			$this->vAlign("center", $colAwal[0].$sr .":".$colAkhir[2].$sr2);
